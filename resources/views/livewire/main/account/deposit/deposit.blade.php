                                ...
                                createOrder: function(data, actions) {
                                    return actions.order.create({
                                        purchase_units: [{
                                            ...
                                            amount    : {
                                                value: "{{ ($amount - $fee_value) * payment_gateway('paypal')?->exchange_rate }}"
                                            }
                                        }]
                                    });
                                },
                        ...
                        <script>
                            window.makeFlutterwavePayment = function() {
                                FlutterwaveCheckout({
                                    ...
                                    amount         : Number("{{ $amount * payment_gateway('flutterwave')?->exchange_rate }}"),
                        ...
                        <script>
                            window.makePaystackPayment = function(){
                                let handler = PaystackPop.setup({
                                    ...
                                    email   : '{{ auth()->user()->email }}',
                                    amount  : Number("{{ $amount * payment_gateway('paystack')?->exchange_rate }}") * 100,
